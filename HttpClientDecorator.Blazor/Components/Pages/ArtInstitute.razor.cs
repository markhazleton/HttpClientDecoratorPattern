...
public partial class ArtInstitute
{
    [Inject]
    private ILogger<ArtInstitute> Logger { get; set; } = default!;

    [Inject]
    private IHttpRequestResultService Service { get; set; } = default!;

    public HttpRequestResult<ArtWorksResponseModel> ArtResponse { get; set; } = default!;
    private ArtWorksResponseModel _artWorksResponse = new();
    private ArtListModel _artList = new();

    // Public properties to expose to the view
    public ArtWorksResponseModel ArtWorksResponse => _artWorksResponse;
    public ArtListModel ArtList => _artList;
    ...
    private async Task LoadArtworks(CancellationToken ct = default)
    {
        ArtResponse = new HttpRequestResult<ArtWorksResponseModel>();
        ...
        ArtResponse = await Service.HttpSendRequestResultAsync(ArtResponse, ct: ct).ConfigureAwait(false);
        ...
        {
            _artWorksResponse = new ArtWorksResponseModel();
            Logger.LogError("ArtResponse.ResponseResults is null");
        }
        else
        {
            _artWorksResponse = ArtResponse.ResponseResults;
            if (_artWorksResponse.data != null)
            {
                foreach (var item in _artWorksResponse.data)
                {
                    if (item != null)
                    {
                        ArtWorkModel artWork = new()
                        {
                            id = item.id.ToString(),
                            title = item.title,
    ...
    public class ArtListModel
    {
        ...
        public List<ArtWorkModel> list = new();
    ...
    public class ArtWorksResponseModel
    {
